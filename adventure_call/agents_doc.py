"""The AGENTS.md written into every ``.adventure-call/`` store (tic-49da).

Kept as a template in the package, not hand-edited in stores: ``update``
rewrites the file, so a store always documents the CLI version that wrote it.
"""
...
Static call/import/data-flow graph of this Python project (adventure-call {version}).
...
def render_agents_md() -> str:
    return _TEMPLATE.format(version=__version__)
